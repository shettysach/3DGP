...
MountainResult computeMountain(const MountainInput& in) {
    ...
    const float mountainCore = smoothstep(0.42f, 0.78f, mountainSignal);
    const float mountainFoot = smoothstep(0.22f, 0.55f, mountainSignal);
    ...
    float height =
        (0.78f * mountainShape + 0.22f * std::pow(mountainShape, 1.45f)) * in.verticalScale * 1.25f +
        mountainRidgeDetail * in.verticalScale * 0.12f;

    const float peakKnee = in.verticalScale * 0.95f;
    if (height > peakKnee) {
