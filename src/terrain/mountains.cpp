...
MountainResult computeMountain(const MountainInput& in) {
    ...
    const float slopeHint =
        std::clamp((in.ridges - 0.35f) * 1.55f + in.detail * 0.2f, 0.0f, 1.0f);

    const float signal = std::clamp(
        in.continental * 0.55f + slopeHint * 0.35f + in.rangeMask * 0.45f,
        0.0f,
        1.0f
    );
    ...
    const float weight =
        std::clamp(0.40f * core + 0.60f * foot, 0.0f, 1.0f) * in.rangeMask;

    const float base =
        0.52f * in.continental + 0.38f * in.ridges + 0.10f * in.detail;
    ...
    float height = (0.78f * shape + 0.22f * std::pow(shape, powExp))
            * in.verticalScale * p.heightScale * 1.316f
