...
namespace {
    ...
    constexpr std::array<EcologyClimateRule, kEcologyCount>
        kEcologyClimateRules = {{
    ...
    void normalizeWeights(EcologyWeightVector& weights) {
        normalizeWeightArray(
            weights.data(),
            weights.size(),
            ecologyIndex(EcologyId::Grassland)
        );
    }
    ...
    void applyMappedEcologyWeights(
        ...
        for (size_t ecology = 0; ecology < count; ecology++) {
            biomeWeights[biomeIndex(mapping[ecology])] +=
    ...
    EcologyWeightVector ecologyWeightsFromClimate(
        ...
        const float effectiveMoisture =
            std::clamp(m + mountainMoistureBonus, 0.0f, 1.0f);
        ...
        weights[ecologyIndex(EcologyId::Steppe)] +=
            dry * (1.0f - hot * 0.50f) * 0.28f;
        weights[ecologyIndex(EcologyId::Grassland)] +=
            (1.0f - dry) * (1.0f - wet * 0.55f) * (1.0f - cold * 0.70f) * 0.18f;
        weights[ecologyIndex(EcologyId::Forest)] +=
            wet * (1.0f - cold * 0.50f) * 0.30f;
        weights[ecologyIndex(EcologyId::Taiga)] += cold * wet * 0.42f;
        weights[ecologyIndex(EcologyId::Tundra)] +=
            cold * (1.0f - smoothstep(0.35f, 0.90f, effectiveMoisture)) * 0.40f;

        if (landform <= LandformId::Foothill) {
            weights[ecologyIndex(EcologyId::Marsh)] += flat * wet
                * smoothstep(0.06f, 0.40f, r) * smoothstep(0.03f, 0.30f, m)
                * 0.85f;
        }
    ...
    BiomeWeightVector biomeWeightsFromEcology(
        ...
        if (landform == LandformId::Alpine) {
            const float snowWeight =
                ...
                * smoothstep(
                    0.28f,
                    0.62f,
                    1.0f - std::clamp(temperature, 0.0f, 1.0f)
                );
        ...
        if (landform == LandformId::Mountain) {
            const float alpineWeight = 0.25f
                    * smoothstep(0.72f,
                                 0.92f,
                                 std::clamp(elevationNorm, 0.0f, 1.0f))
                + 0.20f * eco(EcologyId::Tundra)
                + 0.15f * eco(EcologyId::Taiga);
            weights[biomeIndex(BiomeId::RockyAlpine)] =
                1.0f - std::clamp(alpineWeight, 0.0f, 0.55f);
            weights[biomeIndex(BiomeId::Alpine)] =
                std::clamp(alpineWeight, 0.0f, 0.55f);
            normalizeWeights(weights);
            return weights;
        }
        ...
        if (landform == LandformId::Valley) {
            ...
            const float wetValleyBoost =
                smoothstep(0.42f, 0.85f, std::clamp(moisture, 0.0f, 1.0f));
            weights[biomeIndex(BiomeId::MarshLowland)] +=
                eco(EcologyId::Marsh) * 0.80f + wetValleyBoost * 0.15f;
            weights[biomeIndex(BiomeId::ForestPlain)] +=
                eco(EcologyId::Forest) * 0.16f;
            weights[biomeIndex(BiomeId::GrasslandPlain)] +=
                eco(EcologyId::Grassland) * 0.12f;
            normalizeWeights(weights);
            return weights;
        }

        if (landform == LandformId::Foothill) {
            applyMappedEcologyWeights(
                weights,
                ecology,
                kFoothillBiomeByEcology
            );
            normalizeWeights(weights);
            return weights;
        }
        ...
        if (landform == LandformId::Plateau) {
            ...
            const float dryBonus = smoothstep(
                0.55f,
                0.85f,
                1.0f - std::clamp(moisture, 0.0f, 1.0f)
            );
        ...
        const float marshWeight = eco(EcologyId::Marsh)
                * (landform == LandformId::Lowland ? 1.0f : 0.65f)
            + wetLowlandBoost
                * (landform == LandformId::Lowland ? 0.18f : 0.08f);
    ...
    void applyAggressiveBiomeMajority(
        std::vector<uint8_t>& primaryIds,
        const TerrainFields& fields,
        int passes
    ) {
        constexpr size_t kCount = static_cast<size_t>(BiomeId::Count);
        for (int pass = 0; pass < passes; ++pass) {
            std::vector<uint8_t> next = primaryIds;
            for (int z = 0; z < fields.depth; ++z) {
                const int z0 = std::max(0, z - 2);
                const int z1 = std::min(fields.depth - 1, z + 2);
                for (int x = 0; x < fields.width; ++x) {
                    ...
                    for (int nz = z0; nz <= z1; ++nz) {
                        for (int nx = x0; nx <= x1; ++nx) {
                            ++counts
                                [primaryIds[fieldIndex(nx, nz, fields.width)]];
                        }
                    }
                    size_t majority = static_cast<size_t>(primaryIds[idx]);
...
void computeBiomeFields(TerrainFields& fields) {
    ...
    const float invHeightRange =
        1.0f / std::max(0.0001f, fields.maxHeight - fields.minHeight);
    const size_t cellCount = fields.size();

    for (size_t idx = 0; idx < cellCount; ++idx) {
        const LandformId landform =
        ...
        fields.ecologyIds[idx] =
            static_cast<uint8_t>(dominantEcology(ecologyWeights));

        const float elevationNorm =
            (fields.heights[idx] - fields.minHeight) * invHeightRange;
...
void computeBiomeFieldsWFC(
    ...
    if (!solver.solve(tempHeuristics, moistureHeuristics)) {
        ...
        std::cout
            << "[WFC] WARNING: Solver hit a contradiction it couldn't resolve! Falling back to standard noise generation."
            << std::endl;
