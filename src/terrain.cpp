...
namespace {
    ...
    void
    applyRiverPass(TerrainFields& fields, const TerrainSettings& settings) {
    ...
    HeightGradient sampleHeightGradient(
        ...
        const float dx =
            (hR - hL) / (static_cast<float>(xR - xL) * horizontalScale);
        const float dz =
            (hU - hD) / (static_cast<float>(zU - zD) * horizontalScale);
    ...
    void computeSlopeField(TerrainFields& fields, float horizontalScale) {
        for (int z = 0; z < fields.depth; ++z) {
            for (int x = 0; x < fields.width; ++x) {
                ...
                const HeightGradient gradient = sampleHeightGradient(
                    fields.heights,
                    fields.width,
                    fields.depth,
                    horizontalScale,
    ...
    void buildVertices(
        ...
        mesh.vertices.resize(
            static_cast<size_t>(settings.width)
            * static_cast<size_t>(settings.depth)
        );
    ...
    void buildGridIndices(TerrainMesh& mesh, int width, int depth) {
        mesh.indices.reserve(
            static_cast<size_t>(width - 1) * static_cast<size_t>(depth - 1) * 6
        );
        ...
        for (int z = 0; z < depth - 1; ++z) {
            for (int x = 0; x < width - 1; ++x) {
                ...
                const uint32_t i11 =
                    static_cast<uint32_t>((z + 1) * width + (x + 1));
...
TerrainGenerator::TerrainGenerator(TerrainSettings settings) :
    ...
    baseGraph_ = std::make_shared<graph::CompiledGraph>(
        graph::compile(graph::defaultGraph())
    );
}
...
void TerrainGenerator::setBaseGraph(
    std::shared_ptr<const graph::CompiledGraph> graph
) {
...
TerrainMesh TerrainGenerator::generateMesh() const {
    ...
    const auto stageMs = [](const Clock::time_point& start,
                            const Clock::time_point& end) {
        return std::chrono::duration<double, std::milli>(end - start).count();
    };

    TerrainFields fields =
        graph::execute(*baseGraph_, settings_, noiseContext_);
    const auto baseTerrainDone = Clock::now();
    smoothHeights(
        fields.heights,
        fields.mountainWeights,
        fields.valleyWeights,
        settings_.width,
        settings_.depth
    );
    ...
    std::cout << "[profile] terrain " << settings_.width << 'x'
              << settings_.depth
